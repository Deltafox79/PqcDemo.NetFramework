...
namespace PqcDemo.NetFramework
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            string optKem1 = isIta ? "1. ML-KEM (Cifratura / Scambio Chiavi)" : "1. ML-KEM (Encryption / Key Exchange)";
            string optKem2 = isIta ? "2. HQC (Cifratura / Scambio Chiavi - Code-based)" : "2. HQC (Encryption / Key Exchange - Code-based)";
            string optDsa1 = isIta ? "3. ML-DSA (Firma Digitale su Reticoli)" : "3. ML-DSA (Lattice Digital Signature)";
            string optSlh = isIta ? "4. SLH-DSA (Firma Digitale basata su Hash)" : "4. SLH-DSA (Hash-based Digital Signature)";
            string optFnDsa = isIta ? "5. FN-DSA / FALCON (Firma Digitale Compatta)" : "5. FN-DSA / FALCON (Compact Digital Signature)";
            ...
            while (true)
            {
                ...
                AnsiConsole.MarkupLine("[bold cyan]             PQC Demo Suite v1.2                 [/]");
                AnsiConsole.MarkupLine("[bold cyan]=================================================[/]\n");
                ...
                var algorithm = AnsiConsole.Prompt(
                    new SelectionPrompt<string>()
                        .Title(titleAlgo)
                        .AddChoices(new string[] { optKem1, optKem2, optDsa1, optSlh, optFnDsa, optExit })
                        .EnableSearch()
                ...
                else if (algorithm == optKem2)
                {
                    securityChoices = new string[] {
                        isIta ? "1. HQC-128 (Livello 1 - Equivalente AES-128)" : "1. HQC-128 (Level 1 - AES-128 eq.)",
                        isIta ? "2. HQC-192 (Livello 3 - Equivalente AES-192)" : "2. HQC-192 (Level 3 - AES-192 eq.)",
                        isIta ? "3. HQC-256 (Livello 5 - Equivalente AES-256)" : "3. HQC-256 (Level 5 - AES-256 eq.)",
                        optBack
                    };
                }
                else if (algorithm == optDsa1)
                {
                    securityChoices = new string[] {
        ...
        static void RunHqc(string level)
        {
            HqcParameters param = HqcParameters.hqc128;

            if (level.Contains("192")) param = HqcParameters.hqc192;
            else if (level.Contains("256")) param = HqcParameters.hqc256;

            Console.WriteLine("***************** HQC (" + param.Name.ToUpper().Replace("_", "-") + ") *******************");

            var random = new SecureRandom();
            var keyGenParameters = new HqcKeyGenerationParameters(random, param);
            var hqcKeyPairGenerator = new HqcKeyPairGenerator();
            hqcKeyPairGenerator.Init(keyGenParameters);

            var aliceKeyPair = hqcKeyPairGenerator.GenerateKeyPair();
            var alicePublic = (HqcPublicKeyParameters)aliceKeyPair.Public;
            var alicePrivate = (HqcPrivateKeyParameters)aliceKeyPair.Private;
            var pubEncoded = alicePublic.GetEncoded();
            var privateEncoded = alicePrivate.GetEncoded();
            ...
            var encapsulatedSecret = bobHqcKemGenerator.GenerateEncapsulated(alicePublic);
            var bobSecret = encapsulatedSecret.GetSecret();
            var cipherText = encapsulatedSecret.GetEncapsulation();

            var aliceKemExtractor = new HqcKemExtractor(alicePrivate);
